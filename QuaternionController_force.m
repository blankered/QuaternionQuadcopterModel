...
% velocity.error = [Desired.vi(1) - state.ui;
%                   Desired.vi(2) - state.vi;
%                   Desired.vi(3) - state.wi];
% 
...
Desired.pqrdt = - H * Params.K * quat.xdt.error(2:4) - transpose(FuncWdt)*velocity.error + transpose(WT)*velocity.xdt.error + ...
...
pqrddt   = -sign(quat.xdt.error(1))*(Params.att.pqr.kp*quat.xdt.error (2:4)) + Params.att.pqr.kd*(Desired.pqrdt - pqrdt)

output = [force.total, momentDt.body(1), momentDt.body(2), momentDt.body(3)];
